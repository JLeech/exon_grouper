...
class ExonMatcher
		...
		main_allignement.split("").each_with_index do |seq_1_char, position|
			...
			if seq_1_char == "-" || seq_2_char == "-"
				if seq_1_char == "-" && seq_2_char == "-"
					...
					affine_score += start_gap_flag ? -9 : -2
					start_gap_flag = false
					usual_score += -9
				end
			else
				...
				affine_score += blosum_score.nil? ? 0 : blosum_score
				usual_score += blosum_score.nil? ? 0 : blosum_score
				if global
	...
	def print_statistics
		...
		puts "affine_score: 		#{self.affine_score}"
		puts "usual_score:		#{self.usual_score}"
		puts "added_spaces: 		#{self.added_spaces}"
		puts "matching_spaces: 	#{self.matching_spaces}"
		puts "matching_letters: 	#{self.matching_letters}"
		puts "not_mathing_letters:	#{self.not_mathing_letters}"
		puts "matching_coords:  	#{self.exon_1.get_exons_matching_coords(self.exon_2)}"

		puts "aff/seq1_score: 	#{self.affine_score/seq_1_score.to_f}"
		puts "aff/seq2_score: 	#{self.affine_score/self.seq_2_score.to_f}"		

		puts "length coef: 	#{[exon_1_length,exon_2_length].min.to_f/[exon_1_length,exon_2_length].max}"
		puts "match_letter_1 : 	#{ self.matching_letters.to_f / exon_1_length } "
		puts "match_letter_2 : 	#{ self.matching_letters.to_f / exon_2_length } "
	...
	def print_for_csv
		exon_1_length = self.exon_1.allignement.length
		exon_2_length = self.exon_2.allignement.length
		return [self.sequence_data[:pair_id],
				self.affine_score,
				self.usual_score,
				self.added_spaces,
